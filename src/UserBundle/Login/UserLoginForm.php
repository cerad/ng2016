...
EOT;
    ...
    public function render()
    {
        ...
        $loginGoogle   = $this->router->generate('cerad_user_authen_connect',['providerName' => 'google']);
        $loginFacebook = $this->router->generate('cerad_user_authen_connect',['providerName' => 'facebook']);
...
<form role="form" action="{$loginCheckPath}" method="post">
  <div class="form-group">
    ...
    <input 
      type="text" id="user_login_username" class="form-control" tabIndex="1"
      name="username" value="{$lastUsername}" required placeholder="zAYSO Email" />
  </div>
  <div class="form-group">
    <label for="user_login_password" "> 
      Password
      <a href="{$this->router->generate('user_password_reset_request')}" 
          tabIndex="4"><span style="padding-left: 80px;">Forgot zAYSO Password?</span></a>
    </label>
    <input 
      type="password" id="user_login_password" class="form-control" tabIndex="2"
